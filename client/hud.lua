...
local sheetImage = love.graphics.newImage('https://raw.githubusercontent.com/KashouC/darktheme/8b9cbf21a74a0472fcd31801fe057eaa5229de48/sheet.png')
...
local sheetQuads = {}
do
    local W, H = sheetImage:getDimensions()
    for name, part in pairs(sheetParts) do
        sheetQuads[name] = {}
        if part.w then
            sheetQuads[name].main = love.graphics.newQuad(part.x, part.y, part.w, part.h, W, H)
        elseif part.w1 then
            local x, y, w1, w2, w3, h1, h2, h3 = part.x, part.y, part.w1, part.w2, part.w3, part.h1, part.h2, part.h3
            sheetQuads[name].top_left = love.graphics.newQuad(x, y, w1, h1, W, H)
            sheetQuads[name].top = love.graphics.newQuad(x + w1, y, w2, h1, W, H)
            sheetQuads[name].top_right = love.graphics.newQuad(x + w1 + w2, y, w3, h1, W, H)
            sheetQuads[name].right = love.graphics.newQuad(x + w1 + w2, y + h1, w3, h2, W, H)
            sheetQuads[name].bottom_right = love.graphics.newQuad(x + w1 + w2, y + h1 + h2, w3, h3, W, H)
            sheetQuads[name].bottom = love.graphics.newQuad(x + w1, y + h1 + h2, w2, h3, W, H)
            sheetQuads[name].bottom_left = love.graphics.newQuad(x, y + h1 + h2, w1, h3, W, H)
            sheetQuads[name].left = love.graphics.newQuad(x, y + h1, w1, h2, W, H)
            sheetQuads[name].middle = love.graphics.newQuad(x + w1 - 1, y + h1 - 1, 1, 1, W, H)
        end
    end
end

local function drawPart(name, x, y, w, h, subSlice)
    local quad = sheetQuads[name][subSlice or 'main']
    assert(quad, 'internal error: quad not found -- may need `subSlice`')
    ...
    love.graphics.draw(sheetImage, quad, x, y, 0, w / quadWidth, h / quadHeight)
    return w, h
end

local function drawBoxPart(name, x, y, w, h, noMiddle)
    local part = sheetParts[name]
    local w1, w2, w3, h1, h2, h3 = part.w1, part.w2, part.w3, part.h1, part.h2, part.h3

    -- Corners
    drawPart(name, x, y, w1, h1, 'top_left')
    drawPart(name, x + w - w3, y, w3, h1, 'top_right')
    drawPart(name, x, y + h - h3, w1, h3, 'bottom_left')
    drawPart(name, x + w - w3, y + h - h3, w3, h3, 'bottom_right')

    -- Edges
    drawPart(name, x + w1, y, w - w1 - w3, h1, 'top')
    drawPart(name, x + w1, y + h - h3, w - w1 - w3, h3, 'bottom')
    drawPart(name, x, y + h1, w1, h - h1 - h3, 'left')
    drawPart(name, x + w - w3, y + h1, w3, h - h1 - h3, 'right')

    -- Middle
    if not noMiddle then
        drawPart(name, x + w1, y + h1, w - w1 - w3, h - h1 - h3, 'middle')
    end
end
